...
Most superchill observations occur in February, and March.

Could consider including April.
...
- Keep all data series that begin February 1 and end March 31 (Table 3).
...
potential_superchill_months <- c(2, 3)
...
dat_sc_months <- dat %>% 
  ...
  mutate(
    ...
    min_timestamp_utc = min(timestamp_utc), max_timestamp_utc = max(timestamp_utc),
...
# station-year data series with data in February, and/or March
...
# station-year data series with data in February, AND March,
# and station-year data series with data in February, OR March AND superchill
sc_table2 <- dat_sc_months %>% 
  filter(
    superchill_st_year == TRUE |
      (superchill_st_year == FALSE &
         month_utc_min == 2 & day_min == 1 & month_utc_max == 3 & day_max == 31)
  ) %>% 
  distinct(
    county, station, year_utc, superchill_st_year, 
    min_timestamp_utc, max_timestamp_utc, month_utc_min, day_min, month_utc_max, day_max 
...
`r nrow(sc_table1)` data series with observations in February, and/or March.
...
`r nrow(sc_table2)` data series with observations beginning February 1 and ending March 31 OR with observed superchill.
```{r}
#| message: false
...
`r nrow(excluded)` data series that were filtered out of the analysis because they started after February 1 and/or ended before March 31 and do not include superchill observations.
```{r}
#| message: false
...
Check if there are any large data gaps (> 2 hours) within the February - March data series.
...
Number of superchill observations in February - March for data series that had observed superchill in other months.
...
- Superchill is **only** observed in other months for three station-year data series.
