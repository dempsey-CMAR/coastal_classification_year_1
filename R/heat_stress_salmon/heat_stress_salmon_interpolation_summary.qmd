...
layer_files <- list.files(here("output/interpolation")) %>% 
  ...
  separate(
    file_name, 
    into = c("temperature", "file_name2"), sep = 11, remove = FALSE
  ) %>% 
  filter(temperature == "heat_stress") %>% 
  separate(
    file_name2, 
    into = c(
      NA, NA, "standard_depth", "min_number_years", 
      "costras_res", "range_factor", "dist_power",
      "grid_factor",
      "base_grid",
      "training_seed",
      "extension"), sep = "_", remove = TRUE
  ) %>% 
  mutate(across(.cols = standard_depth:training_seed, .fns = ~as.numeric(.x))) %>% 
  filter(standard_depth == standard_depth)
  
```

# Heat Stress Interpolation (`r standard_depth` m)
...
for (i in seq_along(1:nrow(layer_files))) {
  ...
  m <- leaflet(dat) %>%
    ...
    addRasterImage(
      r_i, colors = heat_stress_pal, 
      opacity = 0.75, method = "ngb",
      group = "Heat Stress Layer"
    ) %>%
    addCircleMarkers(
      dat = dat,
