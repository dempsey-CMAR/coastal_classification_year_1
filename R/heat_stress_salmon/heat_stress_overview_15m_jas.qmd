...
Most high temperature observations occur in August and September.
...
consider <- excluded %>% 
  ...
  filter(month_utc_min == 7, month_utc_max == 9, time_span_days >= 75) %>% 
  select(
...
`r nrow(consider)` data series from Table 5 have at least 75 days of data, with partial data in July or September. Consider including these in the analysis.
```{r}
#| message: false
consider %>% 
  select(-sensor_depth_at_low_tide_m) %>% 
...
- Data series missing observations at the beginning of July and end of September, with a time span of at least 75 days (Table 6).
...
- Time span of at least 75 days[^1].
- Mean temperature in July/September is < 15 degrees C.
...
to_keep1 <- dat_hs_months %>% 
  ...
  filter(time_span_days >= 75, month_utc %in% c(7, 9)) %>% 
...
heat_stress_events <- dat_hs %>%
  ...
  identify_heat_stress_events(
    county, station, heat_threshold = high_temperature
  ) %>%
