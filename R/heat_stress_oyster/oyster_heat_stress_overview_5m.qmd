...
prelim_heat_stress <- readRDS(
  here(paste0("data/heat_stress_events_", 
              high_temperature,
...
- **All** of these observations are from Deep Basin, Inverness County in 2018. This station gets filtered out of the analysis below because the data series starts in August (i.e., there is no data for July).
- Will keep July, August, and September for consistency with other heat stress analysis.
...
table2 <- dat_hs_months %>% 
  filter(month_utc_min == 7, day_min == 1, month_utc_max == 9, day_max == 30) %>% 
  distinct(
    county, station, year_utc, sensor_depth_at_low_tide_m,
    min_timestamp_utc, max_timestamp_utc, 
    month_utc_min, day_min, month_utc_max, day_max
...
- Data series missing observations at the beginning of July and end of September, with a time span of at least `r min_days_to_keep` days (Table 6).
...
- Time span of at least `r min_days_to_keep` days[^1].
...
Stations with partial data in July or September that may still be included in the analysis.
...
to_keep1 <- dat_hs_months %>% 
  ...
  filter(time_span_days >= min_days_to_keep, month_utc %in% c(7, 9)) %>% 
...
# } else {
  ...
  heat_stress_likelihood <- heat_stress_events %>%
    ...
    # get total number of heat stress days
    group_by(county, station, sensor_depth_at_low_tide_m) %>% 
    summarise(n_heat_stress_days = sum(event_duration_days)) %>%
    full_join(n_year, by = join_by(county, station)) %>% 
    # calculate likelihood
    mutate(
