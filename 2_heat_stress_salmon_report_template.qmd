...
```{r}
#| tbl-cap: Number and percent of heat stress days each month.
...
dat <- read_csv(
  here(paste0("output/hs_likelihood_", standard_depth, "_18_24_jas.csv")),
...
raster::writeRaster(
  ...
  filename = here(paste0(
    "output/interpolation/heat_stress_likelihood_",
    standard_depth, "_",
    min_number_years, "_",
    costras_res, "_",
    range_factor, "_",
...
ipdw_r <- ipdw@data
ipdw@srs <- costras@srs # make sure this is correct
...
val_data <- test %>% 
  cbind(val[[2]]) %>% 
  rename("observed likelihood" = validation.data,
...
for (i in 1:dim(rstack_dist)[3]) {
  ...
  # station_i <- "Great Bras d'Or Channel NE-W"
  # 
  # if(station_i == "1307") {
  #   ras_station_i <- "X1307"
  # } else if (station_i == "1012") {
  #   ras_station_i <- "X1012"
  # } else {
