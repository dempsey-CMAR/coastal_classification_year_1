...
# Temperature Layer: Heat Stress for Trout
...
Data from sensors placed ~ `r standard_depth` m below the surface (at low tide) was included in the analysis to reflect typical cage conditions in Nova Scotia. This assumes that if high temperatures occur nearer the surface, all trout in the net-pen will swim down to at least 5 m to avoid the warmest waters. If heat stress occurs at `r standard_depth` m, it is assumed that there is also heat stress nearer the surface, and that biomass density and other constraints prevent trout from swimming  deeper to avoid the warm water.
...
dat2 <- read_xlsx(
  here("data/Satellite_HS_Trout_20C_Table.xlsx")
) %>%
...
raster::writeRaster(
  ...
  filename = here(paste0(
    "output/interpolation/heat_stress_trout_",
    standard_depth, "_",
    min_number_years, "_",
    costras_res, "_",
    range_factor, "_",
...
gof_out <- cbind(temperature = "heat_stress_trout", interp_params, n_training, n_test, gof)
